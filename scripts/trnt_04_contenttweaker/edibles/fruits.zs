...
static overrideFruits as float[][string] = {
...
for fruit in fruits {
    ...
    val foodItem as ItemFood = VanillaFactory.createItemFood(fruit, hunger);
    if (fruits.indexOf(fruit) == 0) {
        COT_UTILS.createCreativeTabWithItem(`${MODPACK.id}.fruits`, foodItem);
    }
